 ...
 *
 * The interactive directory remains the homepage, while this build creates
 * crawlable, no-JavaScript service pages for every effective catalog entry in
 * EN / AR / FR / TR. The sitemap is generated from the same catalog so search
 * discovery cannot drift from what users actually see.
 */
const fs = require('fs');
...
const LANGS = {
    en: {
        dir: 'ltr', name: 'English', locale: 'en_US',
        site: 'Account Deletion & Privacy Guide',
        ...
        eyebrow: 'Official account & privacy resources',
        category: 'Category', difficulty: 'Deletion difficulty', resources: 'Official resources', verified: 'Last verified',
    ...
    ar: {
        dir: 'rtl', name: 'العربية', locale: 'ar_AR',
        site: 'دليل حذف الحسابات والخصوصية',
        ...
        eyebrow: 'روابط رسمية للحساب والخصوصية',
    ...
    fr: {
        dir: 'ltr', name: 'Français', locale: 'fr_FR',
        site: 'Guide de suppression de comptes et confidentialité',
        ...
        eyebrow: 'Ressources officielles de compte et confidentialité',
        category: 'Catégorie', difficulty: 'Difficulté de suppression', resources: 'Ressources officielles', verified: 'Dernière vérification',
...
function alternateLinks(platform = null) {
...
function serviceJsonLd(platform, lang, url) {
    ...
    const items = (platform.resources || []).map((resource, index) => ({
        '@type': 'ListItem',
        position: index + 1,
        name: resourceTitleFor(resource, lang),
        url: resource.url
    }));
    const webpage = {
        '@type': 'WebPage', '@id': url, url,
        name: pageTitle(platform, lang), description: pageDescription(platform, lang), inLanguage: lang,
        isPartOf: { '@type': 'WebSite', name: LANGS[lang].site, url: BASE_URL },
        breadcrumb: { '@id': `${url}#breadcrumb` },
        mainEntity: { '@id': `${url}#resources` }
    };
    if (verified) webpage.dateModified = verified;
    return {
...
function languageLinks(platform) {
...
function relatedLinks(platform, lang) {
    const related = platforms.filter(item => item.id !== platform.id && item.category === platform.category).slice(0, 6);
    return related.map(item => `<a href="${BASE_URL}${lang}/services/${slugFor(item)}/">${escapeHtml(nameFor(item, lang))}</a>`).join('');
}
    ...
    const resourceCards = resources.map(resource => {
        ...
        return `<li class="resource-card">
            ...
            <div class="resource-meta">
                <span>${escapeHtml(resourceTypeFor(resource, lang))}</span>
                ${resource.official !== false ? `<span class="official">${escapeHtml(cfg.official)}</span>` : ''}
                ${resource.verified ? `<span>${escapeHtml(cfg.checked)} ${escapeHtml(resource.verified)}</span>` : ''}
                ${platform.loginRequired !== false ? `<span>${escapeHtml(cfg.signIn)}</span>` : ''}
                ${warning}
            </div>
        </li>`;
    }).join('');
...
<head>${commonHead({ lang, title: pageTitle(platform, lang), description: pageDescription(platform, lang), canonical, alternates: alternateLinks(platform), jsonLd })}
</head>
<body>
...
function serviceIndexJsonLd(lang) {
    ...
    return {
        ...
        '@type': 'CollectionPage',
        url,
        name: LANGS[lang].indexTitle,
        description: LANGS[lang].indexDescription,
        inLanguage: lang,
        mainEntity: {
            '@type': 'ItemList',
            itemListElement: platforms.map((platform, index) => ({
                '@type': 'ListItem', position: index + 1, name: nameFor(platform, lang), url: `${url}${slugFor(platform)}/`
            }))
        }
    };
}
...
function renderServiceIndex(lang) {
    ...
    const cards = [...platforms].sort((a, b) => nameFor(a, lang).localeCompare(nameFor(b, lang))).map(platform => {
        const name = nameFor(platform, lang);
...
<body>
<header class="site-header"><div class="container"><a class="brand" href="${BASE_URL}">${escapeHtml(cfg.site)}</a><div class="header-links"><a href="${BASE_URL}">${escapeHtml(cfg.home)}</a></div></div></header>
<main class="container">
    <p class="eyebrow">${escapeHtml(cfg.eyebrow)}</p>
...
function build() {
    ...
    const sitemap = [{ loc: BASE_URL, lastmod: platforms.map(latestVerified).filter(Boolean).sort().at(-1) || undefined }];

    for (const lang of Object.keys(LANGS)) {
        write(`${lang}/services/index.html`, renderServiceIndex(lang));
        sitemap.push({ loc: `${BASE_URL}${lang}/services/`, lastmod: platforms.map(latestVerified).filter(Boolean).sort().at(-1) || undefined });
        for (const platform of platforms) {
            const slug = slugFor(platform);
            write(`${lang}/services/${slug}/index.html`, renderServicePage(platform, lang));
            sitemap.push({ loc: `${BASE_URL}${lang}/services/${slug}/`, lastmod: latestVerified(platform) || undefined });
        }
    }
    ...
    write('build-report.json', JSON.stringify({ services: platforms.length, languages: Object.keys(LANGS).length, servicePages: platforms.length * Object.keys(LANGS).length, sitemapUrls: sitemap.length }, null, 2) + '\n');

    console.log(`Built ${platforms.length * Object.keys(LANGS).length} localized service pages and ${sitemap.length} sitemap URLs.`);
}
